<template>
    <p class="text-center" style="font-size: 20px; font-weight: bold">Это соревнование {{curCompName}}</p>
    <v-table>
        <thead>
        <tr>
    ...
    methods: {
        async fetchItems(){
            try {
                ...
                const responce = await axios('http://cors-anywhere.herokuapp.com/https://api.football-data.org/v4/competitions/' + id + '/matches', OPTIONS);
                this.matches = responce.data.matches;
                this.curCompName = responce.data.competition.name;
